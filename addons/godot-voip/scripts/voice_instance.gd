...
func _speak(sample_data: PackedFloat32Array):
	...
	received_voice_data.emit(sample_data, multiplayer.get_remote_sender_id())
...
func _process_mic():
	if recording:
		...
		if stereo_data.size() > 0:
			...
			# Sending 2096 bytes unreliably which is above the MTU (1392), this will result in higher packet loss
			_speak.rpc(data)
